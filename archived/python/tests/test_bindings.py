...
def test_batch_construction_and_shapes():
    ...
    assert batch.channels == snek.CHANNELS

    obs = batch.encode()
    assert obs.shape == (8, 2, snek.CHANNELS, 11, 11)
    assert obs.dtype == np.float32
    # Board-mask channel (13) marks every cell in the absolute-coordinate board.
    assert np.all(obs[:, :, 13].sum(axis=(2, 3)) == 11 * 11)
...
def test_move_request_round_trip():
    ...
    assert me == 0
    assert obs.shape == (snek.CHANNELS, 11, 11)
    # My head plane uses absolute board coordinates.
    assert obs[0, 5, 5] == 1.0
    # Food plane (11) marks absolute board coordinate (5,6).
    assert obs[11, 6, 5] == 1.0
    # Health plane (3) broadcasts 90/100.
    assert np.allclose(obs[3], 0.9)
    # Opponent head plane (5) marks absolute board coordinate (1,1).
    assert obs[5, 1, 1] == 1.0
