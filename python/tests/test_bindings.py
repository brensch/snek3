...
def test_batch_construction_and_shapes():
    ...
    obs = batch.encode()
    assert obs.shape == (8, 2, 9, 11, 11)
    assert obs.dtype == np.float32
    # Board-mask channel (8) is all ones.
...
def test_move_request_round_trip():
    ...
    # My head plane has the head at (x=5, y=5): index [channel0][y][x].
    assert obs[0, 5, 5] == 1.0
    # Food plane (6) marks (5,6).
    assert obs[6, 6, 5] == 1.0
    # Health plane (2) broadcasts 90/100.
    assert np.allclose(obs[2], 0.9)
    # Opponent head plane (3) marks (1,1).
    assert obs[3, 1, 1] == 1.0
